...
def plot_tile_size_vs_latency():
    ...
    with plt.style.context("default"):
        ...
        spill_cost = np.where(
            exceeds_sram, 5 * ((sram_usage - SRAM_SIZE_KB) / SRAM_SIZE_KB) ** 2, 0
        )
        ...
        # === Mark SRAM limit ===
        ax.axvline(
            x=sram_boundary_br, color="black", linestyle=":", linewidth=2, alpha=0.7
        )
        ax.annotate(
        ...
        ax.scatter(
            [tile_sizes[optimal_idx]],
            [total_latency[optimal_idx]],
            color=colors[2],
            s=200,
            zorder=5,
            marker="*",
        ...
        if any(exceeds_sram):
            ...
            ax.annotate(
                "Spills to HBM\n(10x slower)",
                xy=(tile_sizes[spill_idx + 1], total_latency[spill_idx + 1]),
                xytext=(
                    tile_sizes[spill_idx + 1] * 1.3,
                    total_latency[spill_idx + 1] * 0.7,
                ),
                fontsize=9,
                color=colors[3],
                ha="left",
        ...
        for _i, (br, sram, lat) in enumerate(
            zip(tile_sizes, sram_usage, total_latency, strict=False)
        ):
        ...
        ax.set_title(
            f"Flash Attention: Tile Size vs Latency Trade-off\nA100 GPU (FP16), d={d}, SRAM={SRAM_SIZE_KB}KB",
            fontsize=14,
        )
        ...
        plt.tight_layout()
        plt.savefig(
            "/home/olivier/ml/image/flash_attention/tile_size_vs_latency.png",
            dpi=150,
            bbox_inches="tight",
        )
        plt.close()
...
def plot_memory_scaling():
    ...
    with plt.style.context("default"):
        ...
        ax1.annotate(
            "A100 HBM (80GB)", xy=(12, 85), fontsize=9, color="black", va="bottom"
        )
        ...
        for i, (_br, usage) in enumerate(zip(br_range, sram_usage, strict=False)):
        ...
        ax2.annotate(
            f"A100 SRAM ({SRAM_SIZE_KB}KB)",
            xy=(5.5, SRAM_SIZE_KB + 10),
            fontsize=10,
            color="black",
            ha="right",
        )
        ...
        plt.suptitle(
            "Flash Attention Memory: HBM vs SRAM\nA100 GPU (FP16), d=64",
            fontsize=14,
            y=1.02,
        )
        plt.tight_layout()
        plt.savefig(
            "/home/olivier/ml/image/flash_attention/memory_scaling.png",
            dpi=150,
            bbox_inches="tight",
        )
        plt.close()
...
def plot_arithmetic_intensity():
    ...
    with plt.style.context("default"):
        ...
        ai_values = np.array(
            [compute_flash_attention_ai(S, d, Br) for S in seq_lengths]
        )
        ...
        ax.set_title(
            f"Roofline Model - A100 GPU (FP16)\nFlash Attention: d={d}, Br={Br} (varying S)",
            fontsize=14,
        )
        ...
        plt.tight_layout()
        plt.savefig(
            "/home/olivier/ml/image/flash_attention/arithmetic_intensity.png",
            dpi=150,
            bbox_inches="tight",
        )
        plt.close()
...
def plot_performance_vs_block_size():
    ...
    with plt.style.context("default"):
        ...
        std_ai_values = np.array(
            [compute_standard_attention_ai(S, d) for S in seq_lengths]
        )
        ...
        for Br, color in zip(Br_values, colors, strict=False):
            ...
            if not exceeds_sram:
                ...
                if saturate_idx > 0:
                    ax.plot(
                        seq_lengths[saturate_idx],
                        pct_peak[saturate_idx],
                        "o",
                        color=color,
                        markersize=8,
                        markeredgecolor="white",
        ...
        ax.set_title(
            f"Flash Attention Performance vs Sequence Length\nA100 GPU (FP16), d={d}, SRAM={SRAM_SIZE_KB}KB",
            fontsize=14,
        )
        ...
        plt.tight_layout()
        plt.savefig(
            "/home/olivier/ml/image/flash_attention/performance_vs_block_size.png",
            dpi=150,
            bbox_inches="tight",
        )
        plt.close()
...
def plot_tile_size_heatmap():
    ...
    with plt.style.context("default"):
        ...
        for i, Br in enumerate(tile_sizes):
            for j, Bc in enumerate(tile_sizes):
                ...
                if exceeds_sram[i, j]:
                    ...
                    min_tile = min(Br, Bc)
                    ai_score = np.log2(min_tile) / np.log2(
                        256
                    )  # 0.25 to 1.0 for 16-256
        ...
        # Normalize to 0-1
        performance = (performance - performance.min()) / (
            performance.max() - performance.min()
        )
        ...
        ax.set_title(
            "Performance Heatmap: Br × Bc Tile Combinations\nA100 GPU (FP16)",
            fontsize=14,
        )
        ...
        rect = mpatches.Rectangle(
            (opt_j - 0.5, opt_i - 0.5), 1, 1, fill=False, edgecolor="black", linewidth=3
        )
        ax.add_patch(rect)

        # Add hatching to cells that exceed SRAM
        for i, _Br in enumerate(tile_sizes):
            for j, _Bc in enumerate(tile_sizes):
                if exceeds_sram[i, j]:
    ...
    plt.tight_layout()
    plt.savefig(
        "/home/olivier/ml/image/flash_attention/tile_size_heatmap.png",
        dpi=150,
        bbox_inches="tight",
    )
    plt.close()
...
def plot_fa1_vs_fa2():
    ...
    with plt.style.context("default"):
        ...
        # Fill the difference
        ax1.fill_between(
            seq_lengths, fa2_gb, fa1_gb, alpha=0.2, color="#2a9d8f", label="FA2 savings"
        )
        ...
        bars1 = ax2.bar(
            ...
            color=[
                c if v > 0 else "white"
                for c, v in zip(cat_colors, fa1_values, strict=False)
            ],
            edgecolor="black",
            alpha=0.7,
            hatch="///",
        ...
        bars2 = ax2.bar(
            ...
            color=[
                c if v > 0 else "white"
                for c, v in zip(cat_colors, fa2_values, strict=False)
            ],
            edgecolor="black",
            alpha=0.9,
        )
        ...
        for bar, val in zip(bars1, fa1_values, strict=False):
        ...
        for bar, val in zip(bars2, fa2_values, strict=False):
            if val == 0:
                label = "0"
            elif val < 1:
                label = f"{val:.1f}"
        ...
        ax2.text(
            0.02,
            0.98,
            f"Total: FA1={fa1_total:.0f}MB, FA2={fa2_total:.0f}MB\nFA2 is {ratio:.1f}× more efficient",
        ...
        plt.suptitle(
            "Flash Attention v1 vs v2: Theoretical HBM Access Comparison\n(Both produce identical output - difference is memory access pattern)",
            fontsize=12,
            y=1.02,
        )
        plt.tight_layout()
        plt.savefig(
            "/home/olivier/ml/image/flash_attention/fa1_vs_fa2.png",
            dpi=150,
            bbox_inches="tight",
        )
        plt.close()
