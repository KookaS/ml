...
class MlpPp(Mlp):
    def __init__(self, rank, batch_size, d_model, d_ff, device):
        ...
        if rank % 2 == 0:
            self.weights = torch.zeros(
                (d_model, d_ff), dtype=torch.float32, device=device
            )
        else:
            self.weights = torch.zeros(
                (d_ff, d_model), dtype=torch.float32, device=device
            )
    ...
    def backward(self, out_grad):
        ...
        if self.rank % 2 == 0:
            ...
            a_grad = einsum(
                "bd,fd->bf", out_grad, self.weights.to(dtype=torch.bfloat16)
            )
