...
class MlpFsdp(Mlp):
    def __init__(self, rank, d_model, d_ff, device):
        ...
        self.w_in = torch.zeros(
            (d_model // dist.get_world_size(), d_ff), dtype=torch.float32, device=device
        )
        self.w_out = torch.zeros(
            (d_ff, d_model // dist.get_world_size()), dtype=torch.float32, device=device
        )
    ...
    def forward(self, x):
        ...
        # ALL-GATHER Win
        w_in_global = torch.zeros(
            (D, F), dtype=torch.bfloat16, device=self.w_in.device
        )  # bf16
        # both tensors just got casted so they are contiguous
        dist.all_gather_into_tensor(
            w_in_global, self.w_in.to(dtype=torch.bfloat16)
        )  # we could cache Win as bf16
        ...
        # ALL-GATHER Wout
        w_out_global = torch.zeros(
            (D, F), dtype=torch.bfloat16, device=self.w_out.device
        )  # bf16
        w_out_local = self.w_out.t().to(
            dtype=torch.bfloat16
        )  # we could cache Wout as bf16
    ...
    def backward(self, out_grad):
        ...
        w_out_grad_local = torch.zeros(
            (Dx, F), dtype=torch.bfloat16, device=self.w_out.device
        )  # bf16[Dx, F]
        w_out_grad = w_out_grad.transpose(0, 1).to(
            dtype=torch.bfloat16
        )  # bf16[D, F]{Ux}
        dist.reduce_scatter_tensor(
            w_out_grad_local, w_out_grad, op=dist.ReduceOp.AVG
        )  # averaging noise from different data samples
        del w_out_grad
        ...
        # REDUCE-SCATTER dWin
        w_in_grad_local = torch.zeros(
            (Dx, F), dtype=torch.bfloat16, device=self.w_in.device
        )  # bf16[Dx, F]
        w_in_grad = w_in_grad.contiguous()
        dist.reduce_scatter_tensor(
            w_in_grad_local, w_in_grad, op=dist.ReduceOp.AVG
        )  # averaging noise from different data samples
        del w_in_grad
        ...
        return {
            "layer_out/weights": w_out_grad_local,
            "layer_in/weights": w_in_grad_local,
            "input": x_grad,
        }
...
def worker_fn(rank, world_size):
    ...
    if rank == 0:
        ...
        # Check Shapes
        print(
            f"Grad Win: {grads['layer_in/weights'].shape} (Expected: {D // world_size}, {F})"
        )
        print(
            f"Grad Wout: {grads['layer_out/weights'].shape} (Expected: {F}, {D // world_size})"
        )
...
if __name__ == "__main__":
    WORLD_SIZE = 4
    mp.start_processes(
        worker_fn, args=(WORLD_SIZE,), nprocs=WORLD_SIZE, join=True, start_method="fork"
    )
