...
# memory allocation
x = jnp.zeros(
    (100, 100), dtype=jnp.bfloat16, device=NamedSharding(mesh, P("X", "Y"))
)  # In[Bx, Dy]
w = jnp.zeros(
    (100, 400), dtype=jnp.bfloat16, device=NamedSharding(mesh, P("Y", None))
)  # W[Dy, F]
...
def main(x, w):
    ...
    out = jax.lax.psum(
        out, axis_name="Y"
    )  # all-reduce accross the contracting dimension, Out[Bx, F]{Uy} -> Out[Bx, F]
...
def main(x, w):
    ...
    out = jax.lax.psum_scatter(
        out, axis_name="Y", scatter_dimension=1, tiled=True
    )  # reduce-scatter accross the contracting dimension, Out[Bx, F]{Uy}
    inspect_array(out, "Out -- Reduce-Scatter")

    out = jax.lax.all_gather(
        out, axis_name="Y", axis=1, tiled=True
    )  # all-gather the results, Out[Bx, F]{Uy} -> Out[Bx, F]
