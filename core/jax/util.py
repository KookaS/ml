...
def inspect_array(arr, name="Array"):
    ...
    for i, (dim_name, dim_size) in enumerate(zip(spec, arr.shape, strict=False)):
        if dim_name is None:
            print(
                f"  - Axis {i} (Size {dim_size}): Replicated (Full copy on all devices)"
            )
        else:
            ...
            print(
                f"  - Axis {i} (Size {dim_size}): Sharded on '{dim_name}' "
