...
def compute_mha_intensity(B, S, N, H, bytes_per_param=2):
    ...
    compute_total = (
        compute_qkv + compute_qk + compute_softmax + compute_attention + compute_out
    )
...
intensities_algo = np.array(
    [compute_mha_intensity(B, S, N, H, bytes_per_param) for S in seq_lengths]
)
